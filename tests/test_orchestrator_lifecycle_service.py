...
class FakeTraceStore:
    ...
    def create_run(self, **kwargs):
        ...
        return TraceContext(run_id="run-1", trace_id="trace-1", task_id=kwargs["task_id"], principal=kwargs["principal"], request_id=kwargs["request_id"])
...
class FakeTaskStore:
    ...
    def create_task(self, **kwargs):
        ...
        return TaskContext(task_id="task-1", root_task_id=kwargs["root_task_id"], parent_task_id=kwargs["parent_task_id"], kind=kwargs["kind"], owner=kwargs["owner"], principal=kwargs["principal"], request_id=kwargs["request_id"], scheduler_task_id=kwargs["scheduler_task_id"], command=kwargs["command"], status=kwargs["status"], run_id="run-1")
...
class OrchestratorLifecycleServiceTests(unittest.TestCase):
    def test_start_and_finish_trace_run(self) -> None:
        ...
        with patch("botboy.orchestrator_lifecycle_service.bind_trace_context", return_value="token") as bind_mock, patch("botboy.orchestrator_lifecycle_service.reset_trace_context") as reset_mock:
            ctx, root_span_id, token = service.start_trace_run("plan task", "alice", "req-1", task_id="task-9")
            ...
            self.assertEqual(bot.trace_store.calls[0], ("create_run", {"command": "plan task", "principal": "alice", "request_id": "req-1", "task_id": "task-9"}))
            self.assertEqual(bot.trace_store.calls[1][0], "start_span")
            service.finish_trace_run(ctx, root_span_id, token, status="success", cmd_type="plan", summary="done", payload_ref="payload-1")
    ...
    def test_create_task_context(self) -> None:
        bot = FakeBot()
        service = create_orchestrator_lifecycle_service(bot)
        with patch.object(service, "persist_security_context") as persist_mock:
            task_ctx = service.create_task_context("  do something important  ", principal="alice", request_id="req-2", kind="command")
        ...
        self.assertEqual(bot.task_store.calls[0]["payload"], {"command": "  do something important  ", "kind": "command"})
        persist_mock.assert_called_once()

    def test_create_task_context_passthrough(self) -> None:
        bot = FakeBot()
        service = create_orchestrator_lifecycle_service(bot)
        existing = TaskContext(task_id="task-2", root_task_id="task-2", parent_task_id="", kind="command", owner="botboy", principal="alice", request_id="req-3", scheduler_task_id="", command="noop", status=TASK_STATUS_QUEUED, run_id="run-2")
    ...
    def test_decorate_with_task(self) -> None:
        task_ctx = TaskContext(task_id="task-3", root_task_id="root-3", parent_task_id="parent-3", kind="command", owner="botboy", principal="alice", request_id="req-4", scheduler_task_id="", command="run", status=TASK_STATUS_QUEUED, run_id="run-3")
        result = {"success": True}
        decorated = OrchestratorLifecycleService.decorate_with_task(result, task_ctx, "running")
        self.assertIs(decorated, result)
        self.assertEqual(decorated["data"]["task"], {"task_id": "task-3", "root_task_id": "root-3", "parent_task_id": "parent-3", "status": "running"})

    def test_trace_async_label_wraps_success_and_error(self) -> None:
        bot = FakeBot()
        service = create_orchestrator_lifecycle_service(bot)
        trace_ctx = TraceContext(run_id="run-async", trace_id="trace-async", task_id="task-async", principal="alice", request_id="req-async")
        async def _success(): return "ok"
        async def _failure(): raise RuntimeError("boom")
        runner = service.trace_async_label(trace_ctx, "parent-span", "worker", "step")
        self.assertEqual(asyncio.run(runner(_success())), "ok")
    ...
    def test_shutdown_resources(self) -> None:
        ...
        self.assertEqual(bot.calls, ["scheduler.stop", "history.close", "memory.close", "trace_store.close", "task_store.close"])
