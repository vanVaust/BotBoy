...
class FakeQueueStore:
    ...
    def __init__(self):
        self.nodes = {
            "node-a": {
                ...
                "worker_id": "worker-a",
                "queue_name": "queue-a",
                "metadata": {"org_id": "tenant-a", "owner_principal": "alice"},
            },
            "node-b": {
                "node_id": "node-b",
                "worker_id": "worker-b",
                "queue_name": "queue-b",
                "metadata": {"org_id": "tenant-b", "owner_principal": "bob"},
            },
        }
        self.queues = {
        ...
        self.leases = {
            "lease-a": {
                ...
                "queue_name": "queue-a",
                "node_id": "node-a",
                "metadata": {"org_id": "tenant-a", "principal": "alice"},
                "lease_status": "active",
            ...
            "lease-b": {
                ...
                "queue_name": "queue-b",
                "node_id": "node-b",
                "metadata": {"org_id": "tenant-b", "principal": "bob"},
                "lease_status": "active",
                "is_recoverable": False,
            },
        }
    ...
    def get_execution_queue(self, queue_name):
    ...
    def renew_queue_lease(self, lease_id, *args, **kwargs):
        return self.leases.get(lease_id)

    def release_queue_lease(self, lease_id, *args, **kwargs):
        return self.leases.get(lease_id)

    def list_worker_nodes(self, **kwargs):
        return list(self.nodes.values())

    def list_execution_queues(self, **kwargs):
    ...
    def worker_node_summary(self):
        return {"available": True, "node_count": 2}

    def queue_summary(self):
        return {"available": True, "queue_count": 2}
...
class QueueTenantSecurityTests(unittest.TestCase):
    ...
    def test_worker_can_acquire_only_owned_node_in_current_tenant(self):
        lease = self.proxy.acquire_queue_lease(
            queue_name="queue-a", node_id="node-a", task_id="task-a", principal="alice"
        )
        ...
        self.assertIsNone(
            self.proxy.acquire_queue_lease(
                queue_name="queue-b", node_id="node-b", task_id="task-b", principal="alice"
            )
        )
    ...
    def test_admin_cannot_cross_tenant(self):
        ...
        with self.assertRaises(Exception):
            self.proxy.acquire_queue_lease(
                queue_name="queue-b", node_id="node-b", task_id="task-b", principal="admin"
            )
    ...
    def test_lease_renew_and_release_are_tenant_bound(self):
        ...
        self.assertIsNone(self.proxy.renew_queue_lease("lease-b", principal="alice"))
        self.assertIsNotNone(self.proxy.release_queue_lease("lease-a", principal="alice"))
        self.assertIsNone(self.proxy.release_queue_lease("lease-b", principal="alice"))
