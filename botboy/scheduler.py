...
class SchedulerStore(_SQLiteMixin):
    def __init__(self,db_path:str=":memory:")->None:self._init_connection_pool(db_path,_SCHEMA)
    def add_task(self,name:str,schedule:str,task_type:str="generic",payload:Optional[dict]=None,max_runs:Optional[int]=None)->str:
        task_id=_secrets.token_hex(8); next_run=parse_schedule(schedule); args=(task_id,name,schedule,task_type,json.dumps(payload or {}),next_run.timestamp(),max_runs,datetime.now(timezone.utc).isoformat()); sql="INSERT INTO scheduled_tasks (task_id,name,schedule,task_type,payload,next_run_ts,enabled,max_runs,run_count,created_at) VALUES (?,?,?,?,?,?,1,?,0,?)"
        if self._is_memory:
...
class BotBoyScheduler:
    _POLL_INTERVAL=10
    def __init__(self,db_path:str=":memory:")->None:self._store=SchedulerStore(db_path); self._handlers:Dict[str,Callable]={}; self._thread:Optional[threading.Thread]=None; self._stop_event=threading.Event(); self._running=False
    def register_handler(self,task_type:str,handler:Callable)->None:self._handlers[task_type]=handler
    def add(self,name:str,schedule:str,payload:Optional[dict]=None,task_type:str="generic",max_runs:Optional[int]=None)->str:return self._store.add_task(name,schedule,task_type=task_type,payload=payload,max_runs=max_runs)
