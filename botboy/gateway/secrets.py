...
def _normalize_principal_id(principal_id: str) -> str:
    value = re.sub(r"[^a-zA-Z0-9._-]+", "-", principal_id.strip())
    value = value.strip(".-_")
    if not value:
...
class SecretStore(_SQLiteMixin):
    ...
    Key format: bbk_{key_id}_{secret}
    Cleartext NEVER stored — only PBKDF2 hash.
    """
    ...
    def validate(self, full_key: str) -> Optional[ApiKey]:
        """Returns ApiKey if valid, None otherwise. Updates use_count."""
    ...
    def rotate(self, key_id: str, label: str = "") -> Optional[ApiKey]:
        """Atomically revoke old key and generate a new one with same role."""
        conn = self._get_conn()
        row = conn.execute(
            "SELECT role, expires_at FROM api_keys WHERE key_id = ? AND revoked = 0", (key_id,)
        ).fetchone()
        if not row:
            return None
        self.revoke(key_id)
        ...
        if row["expires_at"]:
            ...
            ttl = max(remaining, 0.001)
        return self.generate(role=row["role"], label=label or f"rotated-{key_id[:6]}", ttl_hours=ttl)
    ...
    def stats(self) -> dict:
        conn = self._get_conn()
        row = conn.execute(
            "SELECT COUNT(*) as total, SUM(revoked) as revoked_count, SUM(use_count) as total_uses FROM api_keys"
        ).fetchone()
        return {
            "total": row["total"],
            "active": row["total"] - (row["revoked_count"] or 0),
            "revoked": row["revoked_count"] or 0,
...
class PrincipalStore(_SQLiteMixin):
    ...
    def upsert_principal(self, username: str, password: str, role: str = "user", org_id: str = "default") -> Principal:
        ...
        existing = conn.execute(
            "SELECT created_at FROM principals WHERE username = ?",
            (username,),
        ).fetchone()
    ...
    def authenticate(self, username: str, password: str) -> Optional[Principal]:
        ...
        row = self._get_conn().execute(
            "SELECT * FROM principals WHERE username = ? AND disabled = 0",
            (username,),
        ).fetchone()
        if not row:
            return None
        if not _verify_key(password, row["password_hash"]):
            return None
        return self._row_to_principal(row)
    ...
    def get_principal(self, username: str) -> Optional[Principal]:
        username = self._normalize_username(username)
        row = self._get_conn().execute(
            "SELECT * FROM principals WHERE username = ?",
            (username,),
        ).fetchone()
    ...
    def has_principals(self) -> bool:
        row = self._get_conn().execute("SELECT COUNT(*) as total FROM principals").fetchone()
    ...
    def disable_principal(self, username: str) -> bool:
        ...
        conn = self._get_conn()
        conn.execute(
            "UPDATE principals SET disabled = 1, updated_at = ? WHERE username = ?",
            (self._now(), username),
        )
        conn.commit()
        return True
    ...
    def stats(self) -> dict:
        row = self._get_conn().execute(
            "SELECT COUNT(*) as total, SUM(disabled) as disabled_count FROM principals"
        ).fetchone()
        return {
            "total": row["total"],
