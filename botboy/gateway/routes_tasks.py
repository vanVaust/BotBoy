...
def create_task_router(ctx: GatewayAppContext) -> APIRouter:
    ...
    @router.get("/api/v2/workers/queues")
    async def worker_queues_list(request: Request):
        ctx.authorize(request.headers, request.client.host if request.client else "", require_auth=ctx.auth_enabled)
        store = ctx.task_store_or_503()
        return {
            "available": True,
    ...
    async def queue_lease_acquire(request: Request):
        ...
        try:
            lease = store.acquire_queue_lease(
                ...
                principal=principal,
                request_id=getattr(request.state, "request_id", "") or str(payload.get("request_id", "")).strip(),
    ...
    @router.post("/api/v2/workers/leases/claim-next")
    async def queue_lease_claim_next(request: Request):
        principal = ctx.authorize(request.headers, request.client.host if request.client else "", require_auth=ctx.auth_enabled)
        store = ctx.task_store_or_503()
        try:
            payload = await request.json()
    ...
    @router.post("/api/v2/workers/leases/{lease_id}/renew")
    async def queue_lease_renew(request: Request, lease_id: str):
        principal = ctx.authorize(request.headers, request.client.host if request.client else "", require_auth=ctx.auth_enabled)
        store = ctx.task_store_or_503()
        try:
            payload = await request.json()
        ...
        if not fencing_token:
            raise HTTPException(status_code=400, detail="Missing fencing_token")
        try:
    ...
    @router.post("/api/v2/workers/leases/{lease_id}/report")
    async def queue_lease_report(request: Request, lease_id: str):
        principal = ctx.authorize(request.headers, request.client.host if request.client else "", require_auth=ctx.auth_enabled)
        store = ctx.task_store_or_503()
        try:
            payload = await request.json()
        ...
        if isinstance(reported_task, dict):
            ...
            sync_parent = getattr(ctx.bot, "_sync_parent_after_child", None)
    ...
    @router.post("/api/v2/workers/leases/{lease_id}/release")
    async def queue_lease_release(request: Request, lease_id: str):
        principal = ctx.authorize(request.headers, request.client.host if request.client else "", require_auth=ctx.auth_enabled)
        store = ctx.task_store_or_503()
        try:
            payload = await request.json()
    ...
    async def task_reassign(request: Request, task_id: str):
        ...
        return {
            "available": True,
            "reassigned": bool(updated),
            "worker_id": worker_id,
            "task": ctx.decorate_task_record(store, updated) if updated else None,
        }
